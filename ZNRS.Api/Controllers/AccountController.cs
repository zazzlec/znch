...
using ZNRS.Api.ViewModels.Rbac.DncMenu;
...
namespace ZNRS.Api.Controllers
{
    /// <summary>
    /// 
    /// </summary>
    ...
    public class AccountController : ControllerBase
    {
        ...
        public AccountController(ZNRSDbContext dbContext)
        {
...
WHERE P.IsDeleted=0 AND P.Status=1 AND EXISTS (SELECT 1 FROM DncUserRoleMapping AS URM WHERE URM.UserGuid='"+ user.Guid.ToString() + "' AND URM.RoleCode=RPM.RoleCode)";
...
WHERE P.IsDeleted=0 AND P.Status=1";
                ...
                {
                    var permissions = _dbContext.DncPermissionWithMenuMysql.FromSql(sqlPermission).ToList();
                    ...
                    response.SetData(new
                    {
                        access = new string[] { },
                        avator = user.Avatar,
                        user_guid = user.Guid.ToString("D"),
...
LEFT JOIN DncPermission AS P ON P.Code = RPM.PermissionCode
INNER JOIN DncMenu AS M ON M.Guid = P.MenuGuid
WHERE P.IsDeleted=0 AND P.Status=1 AND P.Type=0 AND M.IsDeleted=0 AND M.Status=1 AND EXISTS (SELECT 1 FROM DncUserRoleMapping AS URM WHERE URM.UserGuid='"+ AuthContextService.CurrentUser.Guid.ToString() + "' AND URM.RoleCode=RPM.RoleCode)";
            ...
            var menus = _dbContext.DncMenu.FromSql(strSql).ToList();
