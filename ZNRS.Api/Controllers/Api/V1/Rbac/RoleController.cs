...
namespace ZNRS.Api.Controllers.Api.V1.Rbac
{
    ...
    public class RoleController : ControllerBase
    {
        ...
        public RoleController(ZNRSDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult AssignPermission(RoleAssignPermissionPayload payload)
        {
            ...
            {
                response.SetFailed(ddd.Message);
                return Ok(response);
            }
        ...
        public IActionResult FindListByUserGuid(Guid guid)
        {
            ...
            try
            {
                using (_dbContext)
                {
                    var sql = @"SELECT R.* FROM DncUserRoleMapping AS URM INNER JOIN DncRole AS R ON R.Code=URM.RoleCode WHERE URM.UserGuid={0}";
                    var query = _dbContext.DncRole.FromSql(sql, guid.ToString("D")).ToList();
