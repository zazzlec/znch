...
using ZNRS.Api.Entities.QueryModels.DncPermission;
using ZNRS.Api.Extensions;
using ZNRS.Api.Extensions.AuthContext;
using ZNRS.Api.Models.Response;
using ZNRS.Api.RequestPayload.Rbac.Permission;
...
namespace ZNRS.Api.Controllers.Api.V1.Rbac
{
    ...
    public class PermissionController : Controller
    {
        ...
        public PermissionController(ZNRSDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult PermissionTree(string code)
        {
            ...
            using (_dbContext)
            {
                ...
                var sql = @"SELECT P.Code,P.MenuGuid,P.Name,P.ActionCode,ISNULL(S.RoleCode,'') AS RoleCode,(CASE WHEN S.PermissionCode IS NOT NULL THEN 1 ELSE 0 END) AS IsAssigned FROM DncPermission AS P LEFT JOIN (SELECT * FROM DncRolePermissionMapping AS RPM WHERE RPM.RoleCode={0}) AS S ON S.PermissionCode= P.Code WHERE P.IsDeleted=0 AND P.Status=1";
                if (role.IsSuperAdministrator && !ToolService.DbType.Equals("mysql"))
                {
                    sql = @"SELECT P.Code,P.MenuGuid,P.Name,P.ActionCode,'SUPERADM' AS RoleCode,(CASE WHEN P.Code IS NOT NULL THEN 1 ELSE 0 END) AS IsAssigned FROM DncPermission AS P WHERE P.IsDeleted=0 AND P.Status=1";
                }
                else if (!role.IsSuperAdministrator && !ToolService.DbType.Equals("mysql"))
                {
                    sql = @"SELECT P.Code,P.MenuGuid,P.Name,P.ActionCode,ISNULL(S.RoleCode,'') AS RoleCode,(CASE WHEN S.PermissionCode IS NOT NULL THEN 1 ELSE 0 END) AS IsAssigned FROM DncPermission AS P LEFT JOIN (SELECT * FROM DncRolePermissionMapping AS RPM WHERE RPM.RoleCode={0}) AS S ON S.PermissionCode= P.Code WHERE P.IsDeleted=0 AND P.Status=1";
                }
                //mysql  情况
                else if (role.IsSuperAdministrator && ToolService.DbType.Equals("mysql"))
                {
                ...
                {
                    sql = @"SELECT P.Code,P.MenuGuid,P.Name,P.ActionCode,ifnull(S.RoleCode,'') AS RoleCode,(CASE WHEN S.PermissionCode IS NOT NULL THEN 1 ELSE 0 END) AS IsAssigned FROM DncPermission AS P LEFT JOIN (SELECT * FROM DncRolePermissionMapping AS RPM WHERE RPM.RoleCode={0}) AS S ON S.PermissionCode= P.Code WHERE P.IsDeleted=0 AND P.Status=1";
                }


                if (ToolService.DbType.Equals("mysql"))
                {
                    var permissionList = _dbContext.DncPermissionWithAssignPropertyMysql.FromSql(sql, code).ToList();
    ...
    public static class PermissionTreeHelper
    {
        ...
        public static List<PermissionMenuTree> FillRecursive(this List<PermissionMenuTree> menus, List<DncPermissionWithAssignPropertyMysql> permissions, Guid? parentGuid, bool isSuperAdministrator = false)
        {
            List<PermissionMenuTree> recursiveObjects = new List<PermissionMenuTree>();
            foreach (var item in menus.Where(x => x.ParentGuid == parentGuid))
            {
                var children = new PermissionMenuTree
                {
                    AllAssigned = isSuperAdministrator ? true : (permissions.Where(x => x.MenuGuid == item.Guid.ToString("D")).Count(x => x.IsAssigned == 0) == 0),
                    Expand = true,
                    Guid = item.Guid,
                    ParentGuid = item.ParentGuid,
                    Permissions = permissions.Where(x => x.MenuGuid == item.Guid.ToString("D")).Select(x => new PermissionElement
                    {
                        Name = x.Name,
                        Code = x.Code,
                        IsAssignedToRole = IsAssigned(x.IsAssigned, isSuperAdministrator)
