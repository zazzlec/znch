...
using ZNRS.Api.RequestPayload.Rbac.User;
...
namespace ZNRS.Api.Controllers.Api.V1.Rbac
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public UserController(ZNRSDbContext dbContext, IMapper mapper)
        {
        ...
        public IActionResult ModPass(string old, string pass)
        {
            using (_dbContext)
            {
                ...
                old = CryptoClass.RSAEncryption(old);
                if (!user.Password.Equals(old))
                {
                    response.SetFailed("旧密码错误！");
                    return Ok(response);
                }
                pass = CryptoClass.RSAEncryption(pass);
                user.Password = pass;
        ...
        public IActionResult Create(UserCreateViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                {
                    model.Password = CryptoClass.RSAEncryption(model.Password);
        ...
        public IActionResult Edit(UserEditViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                if (!string.IsNullOrEmpty(model.Password))
                {
                    if (model.Password.Equals(entity.Password))
                    {
