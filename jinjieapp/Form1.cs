...
namespace jinjieapp
{
    public partial class Form1 : Form
    {
        ...
        //  public static string url = "http://172.21.32.160:8040";//接口地址，需要赋值(和利时)
        //public  string url()
        //{
        //    return HttpHelpercs.gethost();
        //}//  "http://132.0.6.80:8721";//接口地址，需要赋值(锦界)
        ...
        public static string nspace = "unit06";
        ...
        public void CHPoint(DBHelper db)
        {
            ...
            // arr.Clear();
            string sql_data = "insert  dncchqpointdata (Name_kw,DncBoilerId,DncBoiler_Name,DncChstatusId,DncChstatus_Name,Lastchtime,position,last_temp_dif_Val,now_temp_dif_Val,slb_floor_Val,slb_circle_num,now_temp_qp_Val,now_temp_bh_Val,realtime) select Name_kw,DncBoilerId,DncBoiler_Name,DncChstatusId,DncChstatus_Name,Lastchtime,position,last_temp_dif_Val,now_temp_dif_Val,slb_floor_Val,slb_circle_num,now_temp_qp_Val,now_temp_bh_Val,realtime from dncchqpoint where DncBoilerId=" + bid;
        ...
        {
            //获取锅炉信息
            string sql_pointkks = "select DISTINCT Name_kw from dncchpointkks_6 where Status=1 and  IsDeleted=0 and DncBoilerId=" + bid;
            ...
            for (int i = 0; i < ja.Count(); i++)
            {
                ...
                if (item["item"]["timestamp"].ToString() == "")
                {
                ...
                else
                {
                    ...
                    up_date = ConvertLongToDateTime(timestamp);//锦界环境
                }

                
                    
                    d1 = up_date;
                sql_up_pvalue = "update dncchpointkks_6 set Pvalue=" + value + ",RealTime='" + up_date + "' where Name_kw='" + name + "' and DncBoilerId=" + bid + ";";
                    arr.Add(sql_up_pvalue);
            ...
            realtime = d1;
            arr.Clear();
            string sql_point_all = "select  Name_kw,DncTypeId,DncType_Name,Pvalue from dncchpointkks_6 where Status=1 and IsDeleted=0 and DncBoilerId=" + bid;
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            if (dt_point != null && dt_point.Rows.Count > 0)
            {
                ...
                if (yq_ratio_real < kyq_yq_ratio_low_Val)
                {
                    ...
                    DateTime lastruntime = DateTime.Parse("2020-12-25 00:00:00");
                    int kyqid = 0;
                    string kyqname = "";
                    string bname = "";
                    ...
                    foreach (DataRow item in dt_kyq.Rows)
                    {
                        ...
                        sql_ky_runlist_add.Append("insert into dncchrunlist_kyq(Name_kw, AddTime, Remarks, Status, IsDeleted, DncChqpointId, DncChqpoint_Name, DncBoilerId, DncBoiler_Name) values('" + kyqname + "', '" + realtime + "', '" + yq_ratio_real + "', 1, 0, " + kyqid + ", '" + kyqname + "', " + bid + ", '" + bname + "'); ");
                        if (item[3].ToString() != null && item[3].ToString() != "")
                        {
        ...
        private void ChRun(DBHelper db)
        {
            ...
            if (rdt.Rows.Count > 0)
            {
                ...
                if (db.GetCommand(ss).Rows.Count == 0)
                {
                    ...
                    sql = "update dncboiler set Qp_bh_update='0'  where DncBoilerId=" + bid;
                    db.CommandExecuteNonQuery(sql);
                }
            }
            //沒有正在执行吹灰的吹灰器
        ...
        private void KyqChRun(DBHelper db)
        {
            ...
            if (rdt.Rows.Count > 0)
            {
                ...
                foreach (DataRow item in rdt.Rows)
                {
                    ...
                    if (DncChstatusId.Equals("1"))
                    {
                        sql = "update dncchrunlist_kyq set OffTime='" + realtime + "',Status=0 where Id=" + item[3].ToString();
                        db.CommandExecuteNonQuery(sql);
                    }
            ...
            if (dt_chtime != null && dt_chtime.Rows.Count > 0)
            {
                ...
                if (reason > 0)
                {
                    ...
                    if (dt_chlist != null && dt_chlist.Rows.Count > 0)
                    {
                        ...
                        sql_runlist.Append("update dncchlist set Status=0 where DncBoilerId=" + bid + ";");
